app.factory('Http', function ($http, $window, Upload) {
  let requestHTTP = function (target, data, request_type = 'text') {
    ...
    if (!target.includes($window.location.origin)) target = $window.location.origin + target;
    ...
    if (typeof data.data !== 'undefined') param_.data = data.data;
